...
def get_data(start_time, end_time, max_interval=3600000):
    ...
    while start_time < end_time:
        ...
        response = requests.post(
            f"http://{station_address}/api/v1/history", data=payload, timeout=15
        ).json()["data"]
        ...
        for i in range(len(response)):
            if i > 0:
                ...
                if delta > 1100:
                    ...
                    average_value = sum(response[i - 1]["z_axis"]) / len(
                        response[i - 1]["z_axis"]
                    )
...
def update(frame):
    try:
        ...
        data = get_data(
            int(start_time_utc.timestamp() * 1000) + ts_offset,
            int(end_time_utc.timestamp() * 1000) + ts_offset,
        )
        bhz_data = make_trace(
            "BHZ", data[0], data[1]["BHZ"], start_time_utc.timestamp()
        )
        ...
        bhz_data.plot(
            ...
            subplots_adjust_left=0.05,
            subplots_adjust_right=1,
            subplots_adjust_top=1,
            subplots_adjust_bottom=0,
        )
...
if __name__ == "__main__":
    ...
    ani = matplotlib.animation.FuncAnimation(
        fig, update, interval=update_interval * 1000, cache_frame_data=False
    )
